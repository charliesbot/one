...
fun FastingDetailsBottomSheet(
  ...
  val currentDate =
    remember(currentEndTime) {
      Instant.ofEpochMilli(currentEndTime).atZone(ZoneId.systemDefault()).toLocalDate()
    }
  ...
  ModalBottomSheet(onDismissRequest = onDismiss, sheetState = sheetState, modifier = modifier) {
    Column(modifier = Modifier.fillMaxWidth().padding(horizontal = 24.dp).padding(bottom = 32.dp)) {
      ...
      TextButton(onClick = { showDeleteConfirmation = true }) {
        ...
        Text(text = stringResource(R.string.delete_entry), color = MaterialTheme.colorScheme.error)
      }
    }
  }
}
