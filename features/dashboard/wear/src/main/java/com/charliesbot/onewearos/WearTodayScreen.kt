...
fun rememberIsLargeScreen(): Boolean {
  ...
  return remember(screenWidthDp) { screenWidthDp >= 225 }
}
...
fun WearTodayContent(
  ...
  val timeLabel =
    if (isFasting) {
      ...
      stringResource(WearR.string.target_duration_hours, currentGoal.durationDisplay)
    }
  ...
  ScreenScaffold {
    Box {
      ...
      Column(
        ...
        AnimatedVisibility(visible = isFasting) {
          TimeButtonActions(
            ...
            onGoalTimeClick = { onNavigateToGoalSelection() },
          )
        }
        TextToggleButton(
          shapes = TextToggleButtonDefaults.animatedShapes(),
          checked = !isFasting,
          onCheckedChange = { if (isFasting) onStopFasting() else onStartFasting() },
