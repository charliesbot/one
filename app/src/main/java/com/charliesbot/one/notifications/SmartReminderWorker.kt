...
class SmartReminderWorker(context: Context, workerParameters: WorkerParameters) :
  ...
  private suspend fun syncToWearOS(suggestedTimeMillis: Long, reasoning: String) {
    try {
      val request =
        PutDataMapRequest.create(DataLayerConstants.SMART_REMINDER_PATH)
          .apply {
            dataMap.putLong(
              DataLayerConstants.SMART_REMINDER_SUGGESTED_TIME_KEY,
              suggestedTimeMillis,
            )
            dataMap.putString(DataLayerConstants.SMART_REMINDER_REASONING_KEY, reasoning)
            dataMap.putLong(
              DataLayerConstants.SMART_REMINDER_TIMESTAMP_KEY,
  ...
  companion object {
    ...
    /**
     * Schedule the daily smart reminder worker. Runs every 24 hours, starting approximately at the
    ...
    /**
     * Trigger an immediate run of the smart reminder calculation. Useful when settings change or
     * user manually requests a refresh.
     */
    fun triggerImmediateRun(context: Context) {
      val workRequest =
        androidx.work
          .OneTimeWorkRequestBuilder<SmartReminderWorker>()
