...
class WatchFastingStateListenerService : BaseFastingListenerService() {
   ...
   */
  private fun handleSmartReminderSync(
    dataEvents: DataEventBuffer,
    smartRemindersEnabledOverride: Boolean?,
  ) {
    for (event in dataEvents) {
      if (
        ...
        val suggestedTimeMillis =
          dataMap.getLong(DataLayerConstants.SMART_REMINDER_SUGGESTED_TIME_KEY, 0L)
        ...
        if (suggestedTimeMillis > System.currentTimeMillis()) {
          when (smartRemindersEnabledOverride) {
            ...
            true -> {
              ...
              Log.d(
                LOG_TAG,
                "WatchListener: Smart reminder notifications scheduled on watch (forced)",
              )
            }
            ...
            null -> {
              ...
              watchServiceScope.launch {
                try {
                  ...
                  Log.e(
                    LOG_TAG,
                    "WatchListener: Failed to schedule smart reminder notifications",
                    e,
                  )
                }
              }
   ...
   *   `/settings` event was present. This is threaded to [handleSmartReminderSync] so it can decide
   *   whether to use the forced scheduling path.
   */
  private fun handleSettingsSync(dataEvents: DataEventBuffer): Boolean? {
    ...
    for (event in dataEvents) {
      if (event.type == DataEvent.TYPE_CHANGED && event.dataItem.uri.path == "/settings") {
        ...
        val smartReminderModeString =
          dataMap.getString("smart_reminder_mode", SmartReminderMode.AUTO.name)
        val smartReminderMode =
          try {
        ...
        watchServiceScope.launch {
          try {
            ...
            settingsRepository.setFixedFastingStartMinutes(
              fixedFastingStartMinutes,
              syncToRemote = false,
            )
