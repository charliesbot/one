...
class GetSuggestedFastingStartTimeUseCaseTest {
  ...
  fun `execute with MOVING_AVERAGE_ONLY mode and enough history uses average`() = runTest {
    ...
    every { settingsRepository.smartReminderMode } returns
      flowOf(SmartReminderMode.MOVING_AVERAGE_ONLY)
  ...
  @Test
  fun `execute with MOVING_AVERAGE_ONLY mode and not enough history falls back to bedtime`() =
    runTest {
  ...
  fun `execute with AUTO mode and not enough history uses bedtime`() = runTest {
    ...
    val now = System.currentTimeMillis()
    val records = listOf(createFastingRecordAtTime(18, 0, now - 1 * 24 * 60 * 60 * 1000L))
  ...
  fun `filters out records older than 14 days`() = runTest {
    ...
    val records =
      listOf(
        ...
        createFastingRecordAtTime(
          12,
          0,
          now - 20 * 24 * 60 * 60 * 1000L,
        ), // 20 days ago (old, should be filtered)
      )
  ...
  private fun createFastingRecordAtTime(
    hour: Int,
    minute: Int,
    approximateMillis: Long,
  ): FastingRecord {
