 ...
 *
 * Priority:
 * 1. If the user has at least [MIN_RECORDS_FOR_AVERAGE] fasting records in the last [HISTORY_DAYS]
 *    days, calculate the average start time from those records.
...
class GetSuggestedFastingStartTimeUseCase(
      ...
      SmartReminderMode.MOVING_AVERAGE_ONLY -> {
        if (hasEnoughHistory) {
          ...
          calculateFromBedtime()
            .copy(
              reasoning =
                "Not enough history (need $MIN_RECORDS_FOR_AVERAGE records). " +
  ...
  /**
   * Calculate the average start time from recent fasting records. Converts each start time to
   * "minutes from midnight" and averages them.
   */
  ...
  /**
   * Calculate suggested start time based on bedtime setting. Suggested start = bedtime - 4 hours.
   */
  ...
  /**
   * Calculate the circular mean of time values (handles wrap-around at midnight). This prevents
   * issues like averaging 23:00 and 01:00 to get 12:00 instead of 00:00.
   */
  ...
  /**
   * Convert minutes from midnight to an absolute timestamp for today (or tomorrow if time has
   * passed).
   */
