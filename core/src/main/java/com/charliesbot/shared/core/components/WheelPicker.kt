...
fun <T> WheelPicker(
  ...
  LaunchedEffect(pagerState) {
    snapshotFlow { pagerState.currentPage }
      ...
      .collect { hapticFeedback.performHapticFeedback(HapticFeedbackType.TextHandleMove) }
  }
  ...
  Box(modifier = modifier, contentAlignment = Alignment.Center) {
    ...
    VerticalPager(
      ...
      WheelPickerItem(pagerState = pagerState, page = page, itemHeight = itemHeight) {
        itemContent(item)
      }
    }
  }
...
private fun WheelPickerItem(
  ...
  Box(
    modifier =
      Modifier.height(itemHeight).fillMaxWidth().graphicsLayer {
        scaleX = scale
        scaleY = scale
        this.alpha = alpha
      },
...
/**
 * Calculate the offset of the page from the current page. Returns 0f when the page is exactly
 * centered, positive/negative based on direction.
 */
...
private fun WheelPickerPreview() {
  ...
  WheelPicker(items = items, initialIndex = 5, onSelectedIndexChange = {}, infiniteScroll = true) {
    item ->
