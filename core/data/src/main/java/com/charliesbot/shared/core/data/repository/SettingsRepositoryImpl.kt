...
class SettingsRepositoryImpl(context: Context, private val dataStore: DataStore<Preferences>) :
  ...
  override val fixedFastingStartMinutes: Flow<Int> =
    dataStore.data
  ...
  override suspend fun setBedtimeMinutes(minutes: Int, syncToRemote: Boolean) {
    try {
      dataStore.edit { prefs -> prefs[PrefKeys.BEDTIME_MINUTES] = minutes }
  ...
  override suspend fun setSmartReminderMode(mode: SmartReminderMode, syncToRemote: Boolean) {
    try {
      dataStore.edit { prefs -> prefs[PrefKeys.SMART_REMINDER_MODE] = mode.name }
  ...
  override suspend fun setFixedFastingStartMinutes(minutes: Int, syncToRemote: Boolean) {
    try {
      dataStore.edit { prefs -> prefs[PrefKeys.FIXED_FASTING_START_MINUTES] = minutes }
  ...
  private suspend fun syncSettingsToRemote() {
    try {
      ...
      val request: PutDataRequest =
        PutDataMapRequest.create(SETTINGS_PATH_KEY)
          .apply {
