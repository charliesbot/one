...
/**
 * The concrete implementation of the [FastingDataRepository] interface, serving as the primary
 * source of truth for all fasting-related data.
 *
 * This class is responsible for two main functions:
 * 1. **Local Persistence:** It uses Jetpack DataStore to save and retrieve the fasting state on the
 *    local device.
 * 2. **Remote Synchronization:** It uses the Wearable Data Layer API (`DataClient`) to sync state
 *    changes to connected devices, ensuring consistency across the network.
 *
...
class FastingDataRepositoryImpl(context: Context, private val dataStore: DataStore<Preferences>) :
  ...
  override val isFasting: Flow<Boolean> =
    dataStore.data
  ...
  override val startTimeInMillis: Flow<Long> =
    dataStore.data
      .catch { exception -> handleDataStoreError(exception, "startTimeInMillis") }
      .map { it[PrefKeys.START_TIME] ?: -1 }
  override val fastingGoalId: Flow<String> =
    dataStore.data
  ...
  override val fastingDataItem: Flow<FastingDataItem> =
    combine(isFasting, startTimeInMillis, fastingGoalId, lastUpdateTimestamp) {
        isFastingValue,
        startTimeValue,
        goalIdValue,
        timestampValue ->
  ...
  override suspend fun getCurrentFasting(): FastingDataItem =
    try {
      Log.d(LOG_TAG, "getCurrentFasting (widget path): Attempting to read DataStore.data.first()")
  ...
  private suspend fun updateLocalStore(
    ...
    try {
      dataStore.edit { prefs ->
        ...
        fastingGoalId?.let { prefs[PrefKeys.FASTING_GOAL_ID] = it }
  ...
  private suspend fun updateRemoteStore(
    ...
    val request: PutDataRequest =
      PutDataMapRequest.create(DataStoreConstants.FASTING_PATH_KEY)
        .apply {
          dataMap.putBoolean(DataStoreConstants.IS_FASTING_KEY, isFasting)
          startTimeInMillis?.let { dataMap.putLong(DataStoreConstants.START_TIME_KEY, it) }
  ...
  private fun handleDataStoreError(exception: Throwable, flowName: String) {
    if (exception is IOException) {
      Log.e(LOG_TAG, "Repo: IOException reading DataStore for $flowName", exception)
      ...
      Log.e(LOG_TAG, "Repo: Unexpected error reading DataStore for $flowName", exception)
      throw exception
    }
  }
